...
<div class="flex justify-center py-10 px-3">
    <div class="w-full max-w-md bg-white px-8 pt-6 pb-6 mb-4 rounded shadow-md">
        <h1 class="mb-8 text-3xl text-center">ユーザー登録</h1>
        <form action="{{ route('register') }}" method="post">
            @csrf
            <div class="mb-6">
                <label for="name" class="block text-gray-darker text-sm font-bold mb-2">ユーザー名</label>
                <input type="text" name="name" id="name" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-darker @error('name') border-red-500 @enderror" placeholder="ユーザー名" value="{{ old('name') }}">
                @error('name')
