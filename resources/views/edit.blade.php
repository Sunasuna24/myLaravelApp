...
<div class="flex justify-center py-10 px-3">
    <div class="w-full max-w-xl bg-white px-8 pt-6 pb-6 mb-4 rounded shadow-md">
        <h1 class="mb-8 text-3xl text-center">編集</h1>
        <form action="{{ route('edit', $post->id) }}" method="post">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="title" class="block text-gray-darker text-sm font-bold mb-2">タイトル</label>
                <input type="text" name="title" id="title" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-darker @error('title') border-red-500 @enderror" placeholder="タイトル" value="{{ $post->title }}">
                @error('title')
            ...
            <div class="mb-4">
                ...
                <textarea name="content" id="content" cols="30" rows="10" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-darker @error('content') border-red-500 @enderror" placeholder="本文を入力">{{ $post->content }}</textarea>
                @error('content')
                <div class="text-red-500 mt-2 text-sm">{{ $message }}</div>
                @enderror
            </div>
